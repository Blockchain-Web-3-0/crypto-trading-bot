...
module.exports = {
    ...
    volumePump: (candles) => {
        ...
        let volSma = SMA.calculate({
            period : 20,
            values : candles.slice(-40).map(b => b.volume),
        })

        let candleSizeSma = SMA.calculate({
            period : 20,
            values : candles.slice(-40).map(v => Math.abs(v.open - v.close)),
        })

        let currentCandle = candles.slice(-1)[0]
        let currentVolumeSma = volSma.slice(-1)[0]
        ...
        return {
            ...
            'roc_ma': Math.abs(Math.abs(candles.slice(-1)[0].open - candles.slice(-1)[0].close)) / candleSizeSma.slice(-1)[0] > 4  ? 'success' : undefined,
        }
    },
}
