...
module.exports = class CandlesFromTrades {
    ...
    async onTrade(exchangeName, trade, symbols = []) {
        ...
        if (this.candles[symbol][roundedTime]) {
            ...
            let candle = this.candles[symbol][roundedTime];
        ...
        let lastCandle = this.lastCandleMap[symbol];
        ...
        let ourCandles = [];
        for (let timestamp in this.candles[symbol]) {
            let candle = this.candles[symbol][timestamp];
        ...
        // delete old candles
        Object.keys(this.candles[symbol]).sort((a, b) => b - a).slice(200).forEach(i => {
        ...
        let symbolCfg = symbols.find(s => s.symbol === symbol);
        if (symbolCfg) {
