...
describe('#bitmex exchange implementation', function() {
    it('positions are extracted', () => {
        let positions = Bybit.createPositionsWithOpenStateOnly(createResponse('ws-positions.json'))
    ...
    it('orders are extracted', () => {
        let orders = Bybit.createOrders(createResponse('ws-orders.json'))
    ...
    it('test that request body for order is created (limit order)', () => {
        let body = Bybit.createOrderBody(Order.createLimitPostOnlyOrder('BTCUSD', 'long', 1337, 0.5))
    ...
    it('test that request body for order is created (limit order) [short]', () => {
        let body = Bybit.createOrderBody(Order.createLimitPostOnlyOrder('BTCUSD', 'short', -1337, 0.5))
        body['order_link_id'] = 'foobar'
    ...
    it('test that request body for order is created (stop order)', () => {
        let body = Bybit.createOrderBody(Order.createStopLossOrder('BTCUSD', 1337, 0.5))
        body['order_link_id'] = 'foobar'
    ...
    it('test that order response is provided', async () => {
        ...
        let bybit = new Bybit(undefined, requestClient, undefined, {'info': () => {}, 'error': () => {}})
        ...
        assert.equal((await bybit.findOrderById('5724145d-cc4e-4efc-a085-c8e8a342a279')).id, '5724145d-cc4e-4efc-a085-c8e8a342a279')
    })
